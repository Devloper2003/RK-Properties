...
export default function PropertyComparison({ projects, selectedIds, onClose, onClear, onRemove }: PropertyComparisonProps) {
  ...
  return (
    <div
      className="fixed inset-0 z-[60] bg-gray-900/50 backdrop-blur-sm flex justify-center items-start sm:items-center p-3 sm:p-6 overflow-y-auto"
      onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}
    >
      <div className="bg-gold-50 w-full max-w-6xl rounded-2xl border border-gold-200 shadow-2xl animate-fade-in my-4 sm:my-0">
        {/* Header */}
        <div className="flex items-center justify-between p-5 sm:p-6 border-b border-gold-200 bg-white/60 rounded-t-2xl">
          <div className="flex items-center gap-3">
            ...
            <span className="px-2.5 py-0.5 bg-gold-100 text-gold-800 text-[10px] font-mono font-bold uppercase tracking-wider rounded-full border border-gold-200">
        ...
        <div className="overflow-x-auto p-4 sm:p-6">
          <table className="w-full min-w-[600px]">
            <thead>
              <tr>
                <th className="text-left text-[10px] font-mono uppercase tracking-widest text-gray-400 pb-4 pr-4 w-40 sticky left-0 bg-gold-50">
                  Attribute
                </th>
                {selectedProjects.map(p => (
                  <th key={p.id} className="text-left pb-4 px-3">
                    <div className="flex flex-col gap-1.5">
                      <span className="font-serif text-sm sm:text-base font-bold text-gray-900 leading-tight">{p.name}</span>
                      <button
            ...
            <tbody>
              {COMPARISON_ROWS.map((row, idx) => (
                <tr key={row.key} className={idx % 2 === 0 ? 'bg-white/40' : 'bg-gold-50/40'}>
                  <td className="text-[10px] font-mono uppercase tracking-widest text-gray-500 py-3 px-3 pr-4 font-bold sticky left-0 bg-inherit">
                    {row.label}
                  </td>
                  {selectedProjects.map(p => (
                    <td key={p.id} className="py-3 px-3 text-xs text-gray-700 align-top leading-relaxed">
                      {row.render(p)}
                    </td>
                  ))}
                </tr>
